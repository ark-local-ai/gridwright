...
财务台账通常不是单张工作表，而是**一整个工作簿**：每天实收、月租金、汇总、滞纳金
工作表之间相互引用；上游改动后，下游未必同步。以下为真实数据的一次体检结果：

- 14 张月度表里，**只有 5 张**引用主数据，其余 9 张是孤岛（全部来自最近一年）
- 主数据被编辑后，3 张表的引用断掉，留下 **113 处 `#REF!`**
- 因此 **71 处账对不上**，散落在 13 张表里

这些问题难以在 Excel 中直接发现 —— Excel 不会指出"这张表按理该引用主数据，却没有引用"，
也不会按**铺位**把跨 13 张表的问题聚到一起。gridwright 针对的即是这一问题。

它做三件事：

1. **核对** —— 检查跨表一致性，按业务对象（铺位、月份）汇总不符项
2. **修改** —— 输入"b50 收到 8 月租金 19354.02"，系统生成待改清单；**经确认后执行**
3. **留痕** —— 每一格改动记录旧值，支持回滚；回滚同样留痕（因此可重做）

**代码执行、模型判断**：体检、定位、影响面均由确定性代码计算，不消耗 token、
可复现；模型仅负责理解修改意图。**规则命中的改动连模型都不调用。**
...
工作台的主体是一份**待核清单**，不是数据本身 —— 按铺位聚合、按差额着色：

```
265  处待核对
     1 个工作簿 · 24 张工作表 · 36 个铺位对不上
...
每个铺位可展开到具体坐标，点击直接跳到那张表的那一格。表的连接、当前表信息与改动账目
默认收成一行摘要，点开才展开——它们不该和待核项争注意力。
